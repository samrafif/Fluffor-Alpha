...
def make_grid(X_data, n_res=20):
    ...
    X_grid = np.concatenate(
        (x_meshgrid.reshape(-1, 1), y_meshgrid.reshape(-1, 1)), axis=1
    )
...
def plot_classifier(
    net, X_data, Y_data, x_meshgrid, y_meshgrid, X_grid, export_path=None
):
    ...
    with plt.style.context("seaborn-white"):
        ...
        plt.scatter(
            X_data[preds == 0, 0], X_data[preds == 0, 1], c="b", zorder=1, edgecolor="k"
        )
        plt.scatter(
            X_data[preds == 1, 0], X_data[preds == 1, 1], c="r", zorder=1, edgecolor="k"
        )
...
X1 = (
    np.random.multivariate_normal([0, 0], [[0.1, 0], [0, 0.1]], size=n_class_size)
    + X1_offset
)
...
X2_test = np.random.multivariate_normal([0, 0], [[0.1, 0], [0, 0.1]], size=n_class_size)
...
x_meshgrid_t, y_meshgrid_t, X_grid_t = make_grid(X_test, n_res=100)
...
for epoch_idx in range(0, n_epochs):
    ...
    accuracy = 1 - np.abs(pred - Y_labels).sum() / 200
...
print("Epoch no. %d" % (epoch_idx + 1))
print("loss: ", loss)
print("accuracy: %1.4f" % accuracy)
tr = plot_classifier(net, X, Y_labels, x_meshgrid, y_meshgrid, X_grid)
ts = plot_classifier(net, X_test, Y_labels, x_meshgrid_t, y_meshgrid_t, X_grid_t)
print(ts - tr)
...
plt.figure(num="Experiment results")
